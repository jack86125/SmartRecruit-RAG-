...
class VectorStore:
    ...
    def _create_or_load_collection(self):
        ...
        # --- 步骤 2: 清理并创建 Collection ---
        if self.client.has_collection(collection_name=collection_name):  # 检查是否存在指定名称的集合
            print(f"集合 '{collection_name}' 已存在，正在删除...")  # 如果集合存在，打印删除提示
            self.client.drop_collection(collection_name=collection_name)  # 删除已存在的集合
            print(f"集合 '{collection_name}' 删除成功。")  # 打印删除成功的提示
...
# 验证store_resume函数（基于脚本__main__中的注释部分，模拟）
collection_name="test_collect_rag"
vector_store = VectorStore()
from langchain.docstore.document import Document  # 导入Document
test_doc = Document(page_content="测试简历内容", metadata={"hash": "test_hash"})
test_chunks = [Document(page_content="块1", metadata={"id": "chunk1", "hash": "test_hash"}),
               Document(page_content="块2", metadata={"id": "chunk2", "hash": "test_hash"})]
test_structured = {"name": "测试", "gender": "男", "age": 30, "work_experience": 5}
try:
    # 先清理旧数据（模拟）
    vector_store.mongo_collection.delete_one({"doc_hash": "test_hash"})
    vector_store.client.delete(collection_name, filter=f"doc_hash == 'test_hash'")
    success = vector_store.store_resume(test_doc, test_chunks, test_structured)
    assert success, "存储失败"
    logger.info("存储验证通过！")
