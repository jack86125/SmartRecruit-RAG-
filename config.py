...
class Config(BaseModel):
    ...
    # Elasticsearch 配置
    ES_HOST: str = Field(default="http://localhost:9200", description="ES 主机")
    ES_INDEX_NAME: str = Field(default="resume_chunks", description="ES 索引名")

    # 模型路径
    MODEL_PATH: str = Field(default=".", description="本地模型路径")
    EMBEDDING_MODEL: str = Field(default=r"models/bge-m3",
                                 description="嵌入模型")
    RERANKER_MODEL: str = Field(default=r"models/bge-reranker-base",
                                description="重排序模型")
